...
```brewer_PrGn.clut``` is an LUT file for ```mricroGL``` and ```surfIce``` to create the brewer color scale for the Expansion maps.
...
```eNKI_meta_create.R``` establishes the lifespan sample used for the replication analysis of the human aging - expansion relationship.
