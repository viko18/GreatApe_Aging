...
chimp_B_dat <- read_csv(file = paste(dat_path, 
                                   "Chimp_meta_QC_u50_n189_Rank_17Chimp_cort_n189_TPM03_rank_17_num_match_Haiko2Juna_expansion.csv",
                                   sep = "/")) %>%
...
chimp_M_dat <- read_csv(file = paste(dat_path, 
                                     "Chimp_meta_QC_u50_n189_Rank_17Chimp_cort_n189_TPM03_rank_17_num_match_MeanMacaque2Juna_expansion.csv",
                                     sep = "/")) %>%
  mutate(statistic = abs(statistic),
         variable = as.factor(1:17)) %>% # make T-stat positive for easier plotting and explaining
...
human_dat <- read_csv(file = paste(dat_path,
                      "eNKI_meta_n474_Rank_17IXI_cort_n480_rank_17_Juna2MNI_expansion.csv",
                      sep = "/")) %>%
  mutate(statistic = abs(statistic),
         variable = as.factor(1:17)) %>% # make T-stat positive for easier plotting and explaining
...
for (perm_H in 1:nperm) {
  ...
  perm_res_H[[perm_H]] <- cor(x=human_dat$statistic, y=sample(human_dat$zscore_def), 
                              method = "pearson")
}
...
ggsave(filename = paste(dat_path, 
                        "IXI_N480_J2M_Rank17_R_perm_10e4_CamCAN651.png",
                        sep = "/"))
...
ggsave(filename = paste(dat_path, 
                        "Chimp_N189_H2J_Rank17_R_perm_10e4.png",
                        sep = "/"))
...
ggsave(filename = paste(dat_path, 
                        "Chimp_N189_MM2J_Rank17_R_perm_10e4.png",
                        sep = "/"))
...
ggsave(filename = paste(dat_path, 
                        "IXI_n480_J2M_rank17_exp_age_eNKI474.png",
                        sep = "/"))
...
ggplot(chimp_MB_dat, aes(x = statistic, y = zscore_def, 
                                color=Species, label = labels_sel)) +
  geom_point(size = 4.5) +
...
# save plot #
ggsave(filename = paste(dat_path, 
                        "Chimp_n189_H2J_M2J_rank17_exp_age.png",
                        sep = "/"))
